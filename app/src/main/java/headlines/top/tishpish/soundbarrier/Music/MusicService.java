...
//            Log.d("dekhi",json);
    ...
    private String[] itItAllValue(JSONArray jay) throws JSONException {
        String url[] = new String[jay.length()];
        audio_id =new String[jay.length()];
        audio_title =new String[jay.length()];
        audio_artist =new String[jay.length()];
        audio_avatar=new String[jay.length()];
        for (int i=0;i<jay.length();i++)
        {
            JSONObject job = jay.getJSONObject(i);
            url[i]= job.getString("url");
            audio_id[i]= job.getString("id");
            audio_artist[i]=job.getString("artist");
            audio_title[i]=job.getString("title");
            audio_avatar[i]=job.getString("avatar");
        }
        url = fixAllUrl(url);
        urlToPlay = url[songPosition_in_the_list];
        Log.d("url yo pl", urlToPlay);
        return url;
    }

    private String[] fixAllUrl(String id[])
    {
        ...
        for (int i=0;i<id.length;i++)
        {
            ...
            {
                id[i] = "http://api.awaza.net/music_files/" + id[i];
    ...
    void updateNotification(String text)
    {
        ...
        //////////////////////////////////////////////////////////////
        views.setTextViewText(R.id.title, audio_title[songPosition_in_the_list]);
